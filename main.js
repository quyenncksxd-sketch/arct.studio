document.addEventListener("DOMContentLoaded", () => {
  ...
  const getServiceDetailContent = (key, lang = window.currentGlobalLang) => {
    ...
    const enToViMap = {
      ...
      "Restaurant & Cafe Design": "Thiết Kế Nhà Hàng & Quán Coffee"
    };
    if (enToViMap[title]) {
      title = enToViMap[title];
    }
    const db = {
      "vi": {
        ...
        "Thiết Kế Nhà Hàng & Quán Coffee": "<p>Thiết kế không gian ẩm thực (F&B) đòi hỏi sự kết hợp tuyệt vời giữa bài toán kinh tế vận hành của chủ đầu tư và khả năng thu hút, giữ chân khách hàng bằng trải nghiệm thị giác ấn tượng.</p><p>ARCT Studio mang đến giải pháp tối ưu:</p><ul><li>Quy hoạch giao thông nội bộ tinh tế phân tách luồng vận hành (Bếp, Service) và luồng khách hàng.</li><li>Định hình bản sắc thương hiệu độc bản (Brand Identity) thông qua ngôn ngữ vật liệu, màu sắc và ánh sáng nghệ thuật.</li><li>Tối ưu hóa mật độ bàn ghế để tối đa hóa doanh thu nhưng vẫn đảm bảo sự riêng tư, sang trọng cho thực khách.</li><li>Tư vấn giải pháp âm học, mùi hương và vi khí hậu chuyên sâu.</li></ul>"
      },
      "en": {
        ...
        "Thiết Kế Nhà Hàng & Quán Coffee": "<p>F&B space design demands an exceptional synergy between investors' operational viability and the ability to attract and retain clients through stellar visual experiences.</p><p>ARCT Studio delivers optimal solutions:</p><ul><li>Intelligent internal flow planning separating operations (Kitchen, Service) from customer journeys.</li><li>Defining a unique brand identity through custom materials, color schemes, and artful lighting.</li><li>Optimizing seating density to maximize revenue while guaranteeing luxury privacy for diners.</li><li>In-depth acoustic, scent, and microclimate consulting.</li></ul>"
      }
    };
